...
//Giving a block number, this function gets ithe neighboors block
int** get_neighboor(int which_block, int n){
	int i,j;
	int tot_blocks=(int)pow(n,2);
	int** first_lev=(int**)calloc(3,sizeof(int));
	for(i=0; i<3; i++)	first_lev[i]=(int*)calloc(3,sizeof(int));
	first_lev[1][1]=which_block;

	if(which_block>tot_blocks | which_block<1){//1st check
...
if((which_block-1)%n==0){//Is left border?
	for(i=0; i<3; i++){
	first_lev[i][0]=0;//I set to zero the element left to left border because they aren't correct
	}
}

if(which_block%n==0){//Is right border?
	for(i=0; i<3; i++){
	first_lev[i][2]=0;//I set to zero the element left to left border because they aren't correct
	}
}
...
int get_avg_brightness_block(CvMat* image, CvPoint2D32f* srcQuad){
	...
	for(y=srcQuad[0].y; y<srcQuad[2].y; y++){
		for(x=top_left; x<top_right; x+=3){//The Y (brightness) coordinates are 0,3,6,9,...,3*x
			...
			//printf("%d ",CV_MAT_ELEM(*dst, uchar, y, x));
			}		
	//printf("\n");
	}
